...
export default function UnStakingForm({poolToken,totalUSDCSupply}: Props) {
  ...
  const maxValue = poolToken.tokenBalance[0]? poolToken.tokenBalance[0].tokensOwnedbByUser: 0; 
  const totalUsdc = totalUSDCSupply? totalUSDCSupply:0;
  const totalSupply = poolToken.totalSupply? poolToken.totalSupply:0;
  const currentStakeTok = poolToken.tokenBalance[0]? poolToken.tokenBalance[0].totalStaked:0;
