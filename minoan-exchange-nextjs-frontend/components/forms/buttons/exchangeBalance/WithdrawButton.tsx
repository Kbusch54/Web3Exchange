...
export default  function  WithdrawButton({value,user,disabled,handleZero}:Props)  {
      ...
      useEffect(() => {
        if (isMounted.current) {
          if(waiting.isError){
            ...
            setLoadingStage((prev) => false);
            isMounted.current = false;
            toast.success(`$${moneyFormatter(value)} Withdrawn ${waiting.data.transactionHash}`, {  duration: 6000 ,position:'top-right'});
