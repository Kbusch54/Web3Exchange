...
const DAOButtonSelection: React.FC<Props> = ({proposal,user,dbData,votesReceived,isHolder,id,votesNeededPercentage}) => {
    return (
        ...
        {dbData?.signers?.includes(user) == true && votesReceived < (votesNeededPercentage) / 10 ** 2 && (
            <button className='text-white text-md  lg:text-xl m-2 bg-amber-400 rounded-3xl px-2 py-1'>Check</button>
        )}
        {(dbData?.signers?.includes(user) == false || dbData?.signers?.includes(user) == undefined) && isHolder == true && votesReceived < (votesNeededPercentage) / 10 ** 2 && (
        ...
        {isHolder == null && votesReceived < (votesNeededPercentage) / 10 ** 2 && (
